...
USAGE(){
    echo -e "$R USAGE:: $Nsudo sh 16-redirectors.sh package1 package2..."
    exit 1
...
do
    ...
    then 
        ...
        dnf install $package -y &>>$LOG_FILE | tee -a &>>$LOG_FILE
        VALIDATE $? "Installing $package"
    else
        echo -e "$package is already $Y installed..nothing to do $N" &>>$LOG_FILE | tee -a &>>$LOG_FILE
    fi        
done  